...
def scraper(url, db):
    ...
    try:
        ...
        table = soup.find(
            "table", class_="table techniques-used background table-bordered"
        )
        if not table:
        ...
        for row in rows:
            ...
            technique_id = (
                technique_id_link.text.strip() if technique_id_link else "N/A"
            )
            technique_url = (
                mitre_url + technique_id_link["href"] if technique_id_link else "N/A"
            )
            ...
            if len(columns) == 5:
                ...
                if sub_technique_id_link:
                    ...
                    sub_technique_name = (
                        columns[3]
                        .text.strip()
                        .replace(columns[3].find("a").text.strip(), "")
                        .strip(": ")
                        .strip()
                    )
                    full_technique_name = (
                        f"{columns[3].find('a').text.strip()}: {sub_technique_name}"
                    )
            ...
            if "N" in technique_id:
                technique_id = (
                    technique_url.split("/")[-2] + "/" + technique_url.split("/")[-1]
                )
...
def create_malware_node(malware_name, malware_description):
    try:
        neo4j_conn = populate_database.Neo4jConnection(
