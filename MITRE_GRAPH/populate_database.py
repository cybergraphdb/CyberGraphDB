...
class Neo4jConnection:
    ...
    def create_hash_node_from_report_file(self, MITRE_family):
        ...
        with open(path + str(MITRE_family), 'r') as file:
            ...
            with self._driver.session() as session:
                session.write_transaction(self._create_hash_node, str(MITRE_family), report_data)

    def create_malware_node(self, name, description):
        with self._driver.session() as session:
            result = session.write_transaction(self._create_malware_node, name, description)
            return result

    def create_technique_node(self, technique_type, technique_name, technique_id, technique_url, ref_url, technique_description):
        with self._driver.session() as session:
            session.write_transaction(self._create_technique_node, technique_type, technique_name, technique_id, technique_url, ref_url, technique_description)

    def create_procedure_node(self, id, procedure, description):
        with self._driver.session() as session:
            session.write_transaction(self._create_procedure_node, id, procedure, description)

    def create_mitigation_node(self, id, mitigation, description):
        with self._driver.session() as session:
            session.write_transaction(self._create_mitigation_node, id, mitigation, description)

    def create_detection_node(self, data_source, data_component, detect_description):
        with self._driver.session() as session:
            session.write_transaction(self._create_detection_node, data_source, data_component, detect_description)

    def create_relationship_malware_technique(self, malware_name, technique_name, technique_use):
        with self._driver.session() as session:
            session.write_transaction(self._create_relationship_malware_technique, malware_name, technique_name, technique_use)

    def create_relationship_technique_mitigation(self, technique_name, mitigation):
        with self._driver.session() as session:
            session.write_transaction(self._create_relationship_technique_mitigation, technique_name, mitigation)

    def create_relationship_technique_procedure(self, technique_name, procedure):
        with self._driver.session() as session:
            session.write_transaction(self._create_relationship_technique_procedure, technique_name, procedure)

    def create_relationship_technique_detection(self, technique_name, data_source):
        with self._driver.session() as session:
            session.write_transaction(self._create_relationship_technique_detection, technique_name, data_source)
    ...
    def _create_detection_node(tx, data_source, data_component, detect_description):
        ...
        RETURN d
        """
        result = tx.run(query, data_source=data_source, data_component=data_component, detect_description=detect_description)
        return result.single() is not None
    ...
    @staticmethod
    def _create_technique_node(tx, technique_type, technique_name, technique_id, technique_url, span_href, technique_description):
        query = """
        MERGE (t:Technique {name: $technique_name})
        ...
        """
        tx.run(query, technique_type=technique_type, technique_name=technique_name, technique_id=technique_id, technique_url=technique_url, span_href=span_href, technique_description=technique_description)
    ...
    @staticmethod
    def _create_relationship_malware_technique(tx, malware_name, technique_name, technique_use):
        query = """
        MATCH (m:Malware {name: $malware_name})
        ...
        """
        tx.run(query, malware_name=malware_name, technique_name=technique_name, technique_use=technique_use)
    ...
    def _create_hash_node(tx, MITRE_family, report_data):
        ...
        result = tx.run(query, 
                        ...
                        popular_threat_suggested_label=report_data['attributes']['popular_threat_classification']['suggested_threat_label'])
        return result.single() is not None
